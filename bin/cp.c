...
int main(int argc, char **argv){
	...
	if(argc != 3 || strcmp(argv[1], "--help") == 0){
		write(1, err_param, sizeof(err_param)-1);// error
		return 1;
	}	
	////////////////////////////////////
	if(stat(argv[1], &sb) == -1){
	...
	//write(1, &sb.st_mode, sizeof(sb.st_mode));// mode
	////////////////////////////////////
	...
	// Moving data until EOF or error
	while((numRead = read(inputFd, buf, BUF_SIZE)) > 0){
		if(write(outputFd, buf, numRead) != numRead)
			write(1, err_write, sizeof(err_write)-1);// error
	}
	//////////////////////// TEST CHMOD
	//if(sys_chmod(argv[2], 0755) == -1){
	//	sys_write(1, err_chmod, sizeof(err_chmod)-1);// error
	//	return 1;
	//}
	///////////////////////
	...
	if(numRead == -1){
		write(1, err_read, sizeof(err_read)-1);// error
		return 1;
	}
	if(close(inputFd) == -1){
		write(1, err_close_input, sizeof(err_close_input)-1);// error
		return 1;
	}
	if(close(outputFd) == -1){
